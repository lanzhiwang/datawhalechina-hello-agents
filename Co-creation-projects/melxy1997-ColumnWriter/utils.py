...
class JSONExtractor:
    ...
    def _extract_from_history(response: str) -> Optional[Dict[str, Any]]:
        ...
        if not json_matches:
            json_matches = re.findall(
                r'(\{"column_title".*?"topics".*?\})', response, re.DOTALL
            )
    ...
    def _rebuild_json_from_fields(json_str: str) -> Optional[Dict[str, Any]]:
        ...
        needs_expansion_match = re.search(
            r'"needs_expansion"\s*:\s*(true|false)', json_str
        )
...
def _try_extract_complete_content(
    text: str, thought: Optional[str], thought_end_pos: int
) -> Optional[str]:
    """
        ...
        if brace_end > brace_start:
            ...
            if '"content"' in potential_json or "'content'" in potential_json:
                json_match = re.search(
                    r'\{.*?"content".*?\}', potential_json, re.DOTALL
                )
    ...
    remaining_text = re.sub(
        r"^(Action|Finish|行动|完成)[:：]\s*", "", remaining_text, flags=re.IGNORECASE
    )
    ...
    if is_complete:
        print(
            f"▸ 检测到完整正文内容（长度: {content_length} 字符），自动添加 Finish 前缀"
        )
