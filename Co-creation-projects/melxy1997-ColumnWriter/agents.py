...
from hello_agents import HelloAgentsLLM, ReActAgent, ReflectionAgent, PlanAndSolveAgent
...
from prompts import (
    ...
    get_react_writer_prompt,
    get_reflection_writer_prompts,
    get_planner_prompts,
)
...
""",
        ...
        class CachedExecutor(Executor):
            ...
            def _load_step_from_cache(
                self, step_index: int, step_content: str
            ) -> Optional[str]:
                """从缓存加载步骤结果"""
                ...
                try:
                    ...
                    # 验证缓存的主题和步骤是否匹配
                    if (
                        cache_data.get("topic") == self.main_topic
                        and cache_data.get("step_index") == step_index
                        and cache_data.get("step_content") == step_content
                    ):
            ...
            def _save_step_to_cache(
                self, step_index: int, step_content: str, result: str
            ):
                """保存步骤结果到缓存"""
                cache_file = self._get_step_cache_key(step_index, step_content)
                try:
            ...
            def execute(self, question: str, plan: List[str], **kwargs) -> str:
                ...
                for i, step in enumerate(plan, 1):
                    ...
                    print(
                        f"▸ 步骤 {i} 已完成，结果: {final_answer[:100] if len(final_answer) > 100 else final_answer}..."
                    )
        ...
        self.agent = PlanAndSolveAgent(
            name="专栏规划专家", llm=self.llm, custom_prompts=planner_prompts
        )
    ...
    def _extract_json(self, response: str) -> Dict[str, Any]:
        ...
        try:
            return JSONExtractor.extract(
                response, required_fields=["column_title", "topics"]
            )
        except Exception as e:
    ...
    def run(self, question: str):
        ...
        try:
            ...
            if hasattr(self.agent, "current_history"):
                original_history = (
                    self.agent.current_history.copy()
                    if self.agent.current_history
                    else []
                )
            elif hasattr(self.agent, "history"):
                original_history = (
                    self.agent.history.copy() if self.agent.history else []
                )
            else:
                original_history = []
...
class WriterAgent:
    ...
    def _setup_search_tool(self):
        ...
        try:
            ...
            if os.environ.get("GITHUB_PERSONAL_ACCESS_TOKEN"):
                ...
                print(
                    "▸️  未配置 GITHUB_PERSONAL_ACCESS_TOKEN，跳过 GitHub MCPTool 注册"
                )
        except Exception as e:
    ...
    def _register_search_wrappers(self):
        ...
        def search_code_examples(technology: str, task: str) -> str:
            ...
            return str(
                self.search_tool.run(
                    {"query": f"{technology} {task} code examples tutorial"}
                )
            )
        ...
        self.tool_registry.register_function(
            "web_search", "通用网页搜索，获取最新资讯和资料", web_search
        )
        self.tool_registry.register_function(
            "search_recent_info", "搜索最新信息和动态", search_recent_info
        )
        self.tool_registry.register_function(
            "search_code_examples", "搜索代码示例和教程", search_code_examples
        )
        self.tool_registry.register_function(
            "verify_facts", "验证事实准确性", verify_facts
        )
...
"""
        ...
        try:
            ...
            print(
                f"   {response[:500] if response and len(response) > 500 else response}"
            )
            print()
            ...
            if not response or (isinstance(response, str) and not response.strip()):
                ...
                if self.agent.last_raw_responses:
                    last_raw = self.agent.last_raw_responses[-1]
                    print(
                        f"   尝试从最后一次原始响应中提取内容（长度: {len(last_raw)} 字符）..."
                    )
                    # 尝试直接提取 JSON
                    try:
                ...
                return self._generate_content_with_history(
                    ...
                    structure_requirements,
                    word_count,
                    self.agent.last_history,
                    task_description,
                )

            # 检查是否是错误消息
            if (
                "无法在限定步数内完成" in response
                or "抱歉" in response
                or "流程终止" in response
            ):
                ...
                if self.agent.last_raw_responses:
                    ...
                    try:
                        ...
                        if "content" not in content_data:
                            ...
                            raise ValueError("提取的 JSON 缺少 'content' 字段")
                        print(
                            "▸ 成功从原始响应中提取到内容（尽管 ReActAgent 返回了错误消息）"
                        )
                        return content_data
                    except Exception as e:
                        print(f"   ▸️  从原始响应提取失败: {e}")
            ...
            if response.strip() in ["JSON内容", "JSON", "内容"]:
                print(
                    f"▸️  ReActAgent 返回了占位符 '{response}'，尝试从原始响应中提取..."
                )
                if self.agent.last_raw_responses:
                    last_raw = self.agent.last_raw_responses[-1]
                    print(
                        f"   从最后一次原始响应中提取（长度: {len(last_raw)} 字符）..."
                    )
                    try:
                        content_data = self._extract_json(last_raw)
            ...
            if "content" not in content_data:
                ...
                if self.agent.last_raw_responses:
                    last_raw = self.agent.last_raw_responses[-1]
                    print(
                        f"   尝试从最后一次原始响应中提取（长度: {len(last_raw)} 字符）..."
                    )
                    try:
                        content_data = self._extract_json(last_raw)
                        if isinstance(content_data, dict) and "content" in content_data:
...
"""
    ...
    def revise_content(
        self, original_content: str, review_result: ReviewResult, level: int
    ) -> Dict[str, Any]:
        """
        根据评审意见修改内容
...
class ReviewerAgent:
    ...
    def review_content(
        self, content: str, level: int, target_word_count: int, key_points: List[str]
    ) -> "ReviewResult":
        """
        评审内容
    ...
    def revise_content(
        ...
        if current_word_count < word_count_min:
            word_count_adjustment = (
                f"需要增加约 {word_count_min - current_word_count} 字"
            )
        elif current_word_count > word_count_max:
            word_count_adjustment = (
                f"需要删减约 {current_word_count - word_count_max} 字"
            )
        else:
        ...
        # 格式化评审信息
        strengths = "\n".join(
            [f"- {s}" for s in review_result.detailed_feedback.get("strengths", [])]
        )
        issues = "\n".join(
            [
        ...
        print(f"▸ 修改完成")
        print(
            f"   修改后字数: {revised_data.get('word_count', len(revised_data.get('revised_content', '')))}"
        )
...
""",
    ...
    def generate_and_refine_content(
        self, node: ContentNode, context: Dict[str, Any], level: int
    ) -> Dict[str, Any]:
        """
        生成并反思优化内容
