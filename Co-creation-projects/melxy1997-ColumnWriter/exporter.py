...
class ColumnExporter:
    ...
    def export_to_files(column_data: Dict[str, Any], output_dir: str = "column_output"):
        ...
        for article in column_data["articles"]:
            ...
            with open(filepath, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"- **评审分数**: {article['metadata'].get('review_score', 'N/A')}\n"
                )
                f.write(
                    f"- **评审等级**: {article['metadata'].get('review_grade', 'N/A')}\n"
                )
                ...
                if article.get("has_revisions"):
                    ...
                    if "revision_summary" in article["metadata"]:
                        f.write(f"- **主要修改**:\n")
                        for change in article["metadata"]["revision_summary"].get(
                            "major_changes", []
                        ):
                            f.write(f"  - {change}\n")
    ...
    def _export_report(column_data: Dict[str, Any], filepath: str):
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- **目标读者**: {column_data['column_info']['target_audience']}\n"
            )
            ...
            if "approval_rate" in stats:
                f.write(
                    f"- **直接通过**: {stats.get('approved_nodes', 0)} ({stats['approval_rate']})\n"
                )
                f.write(
                    f"- **修改优化**: {stats.get('revised_nodes', 0)} ({stats['revision_rate']})\n"
                )
            ...
            if "quality_report" in column_data:
                ...
                f.write(
                    f"- **分数范围**: {quality['min_score']}-{quality['max_score']}\n"
                )
                ...
                for grade, count in quality["grade_distribution"].items():
                    if count > 0:
                        percentage = (
                            count / quality["total_evaluated"] * 100
                            if quality["total_evaluated"] > 0
                            else 0
                        )
            ...
            if "creation_stats" in column_data:
                ...
                if creation.get("start_time") and creation.get("end_time"):
                    ...
                    if isinstance(start_time, datetime) and isinstance(
                        ...
                        f.write(
                            f"- **总耗时**: {duration:.1f} 秒 ({duration/60:.1f} 分钟)\n"
                        )
