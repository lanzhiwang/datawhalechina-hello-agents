...
class ChapterGenerateAgent:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.generate_agent = SimpleAgent(
            name="章节生成助手",
            llm=llm,
            system_prompt="你是一位擅长长篇小说结构与文本细化的专业作者助理。",
        )
        self.review_agent = SimpleAgent(
            name="章节审核助手",
            llm=llm,
            system_prompt="你是一位专业的小说审核助手，负责检查章节是否符合小说的结构和风格。",
        )
    ...
    def _ensure_tool(self, novel_id: str, novel_title: str = None):
        if not self.note_tools.get(novel_id):
            if not novel_title:
                raise ValueError(
                    f"Tool for novel_id {novel_id} not initialized and novel_title not provided."
                )
            self.note_tools[novel_id] = NoteTool(
                workspace=os.path.join(
                    self.workspace, f"{novel_title}-{novel_id}", "chapters"
                )
            )
            ...
            if os.path.exists(file_path):
                ...
                self.memories[novel_id].append(
                    MemoryItem(
                        ...
                        summary=(
                            note["tags"][0] if note.get("tags") and note["tags"] else ""
                        ),
    ...
    def run(self, user_input: str, **kwargs) -> str:
        ...
        while steps < self.max_steps:
            ...
            try:
                ...
                # 检查是否包含必要字段
                if (
                    "title" not in response_data
                    or "content" not in response_data
                    or "next_chapter_prediction" not in response_data
                    or "summary" not in response_data
                ):
                    raise ValueError(
                        "JSON 响应缺少必要字段 'title' 或 'content' 或 'next_chapter_prediction' 或 'summary'"
                    )
        ...
        self.memories[novel_id].append(
            MemoryItem(
                ...
                next_chapter_prediction=response_data.get(
        ...
        if is_first_chapter:
            ...
            context = prompt_template.format(
                ...
                chapter_history=(
                    ...
                    else response_data.get("content", "无")
                ),
            ...
            context = prompt_template.format(
                ...
                or [
                    (
                        self.memories[novel_id][-1].next_chapter_prediction
                        if self.memories[novel_id]
                        else "无"
                    )
                ][0],
                chapter_length=chapter_length,
    ...
    def get_prev_summaries(self, novel_id: str):
        if self.memories.get(novel_id):
            return "\n".join(
                [
                    f"【{mem.title}】\n{mem.summary}"
                    for mem in self.memories[novel_id][-self.num_chapter_memories :]
                ]
            )
        return "无"
    ...
    def del_chapter(self, novel_id: str, note_id: str, novel_title: str = None):
        ...
        if self.memories.get(novel_id):
            self.memories[novel_id] = [
                mem for mem in self.memories[novel_id] if mem.node_id != note_id
            ]

    def update_chapter(
        self, novel_id: str, note_id: str, novel_title: str = None, **kwargs
    ):
        """更新章节"""
        if novel_title:
            self._ensure_tool(novel_id, novel_title)
        ...
        if self.memories.get(novel_id):
            for mem in self.memories[novel_id]:
                if mem.node_id == note_id:
                    ...
                    mem.next_chapter_prediction = kwargs.get(
                        "next_chapter_prediction", mem.next_chapter_prediction
                    )
