...
class Config(BaseModel):
    ...
    observation_summary_threshold: int = Field(
        default=2000, gt=0, description="工具输出摘要阈值"
    )

    # ==================== 上下文配置 ====================
    context_max_tokens: int = Field(
        default=8000, gt=0, description="上下文最大 token 数"
    )
    context_reserve_ratio: float = Field(
        default=0.15, ge=0.0, le=0.5, description="生成预留比例"
    )
    ...
    terminal_max_output_size: int = Field(
        default=10 * 1024 * 1024, gt=0, description="终端输出最大大小"
    )
    terminal_confirm_dangerous: bool = Field(
        default=True, description="危险命令需要确认"
    )
    terminal_allow_shell_mode: bool = Field(default=True, description="允许 Shell 模式")
    context_fetch_max_tokens: int = Field(
        default=800, gt=0, description="单个数据源最大 token"
    )
    context_fetch_context_lines: int = Field(
        default=5, ge=0, description="代码上下文行数"
    )

    # ==================== 补丁执行器配置 ====================
    patch_max_files: int = Field(default=10, gt=0, description="单个补丁最大文件数")
    patch_max_total_lines: int = Field(
        default=800, gt=0, description="单个补丁最大总行数"
    )
    ...
    large_change_threshold_files: int = Field(
        default=6, gt=0, description="大规模变更文件数阈值"
    )
    large_change_threshold_lines: int = Field(
        default=400, gt=0, description="大规模变更行数阈值"
    )
    ...
    def from_env(cls, **overrides) -> "Config":
        ...
        env_config = {
            ...
            or os.getenv("CODE_AGENT_DEBUG", "false").lower() == "true",
            "log_level": os.getenv("LOG_LEVEL", "INFO"),
            "temperature": float(os.getenv("TEMPERATURE", "0.7")),
            "helloagents_dir": os.getenv(
                "HELLOAGENTS_DIR", os.getenv("CODE_AGENT_STATE_DIR", ".helloagents")
            ),
            "max_react_steps": int(
                os.getenv(
                    "CODE_AGENT_MAX_REACT_STEPS",
                    os.getenv("CODE_AGENT_MAX_STEPS", "20"),
                )
            ),
            ...
            "patch_max_total_lines": int(
                os.getenv("CODE_AGENT_PATCH_MAX_LINES", "800")
            ),
        }
