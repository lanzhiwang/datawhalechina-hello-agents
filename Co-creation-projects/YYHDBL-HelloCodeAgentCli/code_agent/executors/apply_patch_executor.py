...
class ApplyPatchExecutor:
    ...
    def apply(self, patch_text: str) -> ApplyResult:
        ...
        if len(set(touched_files)) > self.max_files:
            raise PatchApplyError(
                f"Too many files in patch: {len(set(touched_files))} > {self.max_files}"
            )
        ...
        if total_changed > self.max_total_changed_lines:
            raise PatchApplyError(
                f"Patch too large: {total_changed} changed lines > {self.max_total_changed_lines}"
            )
    ...
    def _safe_path(self, rel_path: str) -> Path:
        ...
        if (
            not str(target).startswith(str(self.repo_root.resolve()) + os.sep)
            and target != self.repo_root.resolve()
        ):
    ...
    def _apply_update_payload(
        self, original: List[str], payload: str, rel_path: str
    ) -> List[str]:
        """
        应用 Update File 的内容。
    ...
    def _apply_hunk(
        self, current: List[str], hunk_lines: List[str], rel_path: str
    ) -> List[str]:
        """
        ...
        if idx is None:
            ...
            raise PatchApplyError(
                "Patch hunk context not found; file changed?", recheck_targets=[hint]
            )

        return current[:idx] + after + current[idx + len(before) :]

    def _find_subsequence(
        self, haystack: List[str], needle: List[str]
    ) -> Optional[int]:
        """
        在文件内容中查找代码块的起始位置。
