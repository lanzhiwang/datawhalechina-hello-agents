...
def analyze_gender_preferences(input: str) -> dict:
    ...
    plt.bar(
        gender_counts_dict.keys(), gender_counts_dict.values(), color=["blue", "pink"]
    )
    ...
    plt.figure(figsize=(8, 5))
    plt.bar(
        gender_spending_dict.keys(),
        gender_spending_dict.values(),
        color=["blue", "pink"],
    )
...
def analyze_age_preferences(input: str) -> dict:
    ...
    plt.figure(figsize=(8, 5))
    plt.bar(
        age_group_counts_dict.keys(), age_group_counts_dict.values(), color="skyblue"
    )
...
def analyze_spending_differences(input: str) -> dict:
    # 按性别和年龄段分组统计
    gender_age_spending = (
        df.groupby(["Gender", "Age Group"])["Purchase Amount (USD)"].mean().unstack()
    )
    gender_age_spending_dict = gender_age_spending.to_dict()

    # 按商品类别和年龄段分组统计
    category_age_spending = (
        df.groupby(["Category", "Age Group"])["Purchase Amount (USD)"].mean().unstack()
    )
...
def analyze_subscription_impact(input: str) -> dict:
    ...
    # 3. 之前购买次数对比
    prev_purchases_by_subscription = (
        df.groupby("Subscription Status")["Previous Purchases"]
        .agg(["mean", "std", "count"])
        .round(2)
    )
    ...
    if "Frequency of Purchases" in df.columns:
        ...
        # 转换为数值频率
        df["Purchase_Frequency_Numeric"] = df["Frequency of Purchases"].map(
            frequency_mapping
        )

        frequency_by_subscription = (
            df.groupby("Subscription Status")["Purchase_Frequency_Numeric"]
            .agg(["mean", "std", "count"])
            .round(2)
        )
    ...
    if len(subscribed) > 0 and len(not_subscribed) > 0:
        # 购买金额的效应大小
        mean_diff_amount = (
            subscribed["Purchase Amount (USD)"].mean()
            - not_subscribed["Purchase Amount (USD)"].mean()
        )
        pooled_std_amount = np.sqrt(
            (
                subscribed["Purchase Amount (USD)"].std() ** 2
                + not_subscribed["Purchase Amount (USD)"].std() ** 2
            )
            / 2
        )
        cohens_d_amount = (
            mean_diff_amount / pooled_std_amount if pooled_std_amount > 0 else 0
        )

        # 之前购买次数的效应大小
        mean_diff_prev = (
            subscribed["Previous Purchases"].mean()
            - not_subscribed["Previous Purchases"].mean()
        )
        pooled_std_prev = np.sqrt(
            (
                subscribed["Previous Purchases"].std() ** 2
                + not_subscribed["Previous Purchases"].std() ** 2
            )
            / 2
        )
    ...
    if "Yes" in subscription_counts and "No" in subscription_counts:
        ...
        avg_revenue_per_customer_not_subscribed = total_revenue_not_subscribed / len(
            not_subscribed
        )
        ...
        value_analysis = {
            "total_revenue": {
                ...
                "ratio": (
                    round(total_revenue_subscribed / total_revenue_not_subscribed, 2)
                    if total_revenue_not_subscribed > 0
                    else "N/A"
                ),
            },
            "avg_revenue_per_customer": {
                ...
                "difference": round(
                    avg_revenue_per_customer_subscribed
                    - avg_revenue_per_customer_not_subscribed,
                    2,
                ),
            },
        }
    ...
    # 9. 类别购买差异分析（按订阅状态）
    category_analysis = {}
    category_by_subscription = (
        df.groupby(["Subscription Status", "Category"]).size().unstack(fill_value=0)
    )
    ...
    for category in category_by_subscription.columns:
        ...
        if total_category > 0:
            subscribed_pct = (
                (category_by_subscription.loc["Yes", category] / total_category * 100)
                if "Yes" in category_by_subscription.index
                else 0
            )
            not_subscribed_pct = (
                (category_by_subscription.loc["No", category] / total_category * 100)
                if "No" in category_by_subscription.index
                else 0
            )
            category_analysis[category] = {
                ...
                "subscribed_count": (
                    int(category_by_subscription.loc["Yes", category])
                    if "Yes" in category_by_subscription.index
                    else 0
                ),
                "not_subscribed_count": (
                    int(category_by_subscription.loc["No", category])
                    if "No" in category_by_subscription.index
                    else 0
                ),
            }
    ...
    results = {
        "basic_stats": {
            ...
            "subscribed_percentage": (
                round(subscription_counts.get("Yes", 0) / len(df) * 100, 1)
                if len(df) > 0
                else 0
            ),
        },
...
def analyze_seasonal_preferences(input: str) -> dict:
    ...
    df["Season"] = (
        ...
        .str.strip()
        .str.lower()
        .map(lambda x: season_mapping.get(x, x))
    )
    ...
    for season in valid_seasons:
        ...
        # 按类别统计
        category_stats = (
            season_data.groupby("Category")
            .agg({"Purchase Amount (USD)": ["count", "mean", "sum", "std"]})
            .round(2)
        )
        ...
        # 找出该季节的热销品类（按购买量）
        top_categories_by_count = category_stats.nlargest(3, "count")[
            ["Category", "count"]
        ].to_dict("records")
        top_categories_by_revenue = category_stats.nlargest(3, "total_amount")[
    ...
    for category in all_categories:
        ...
        for season in valid_seasons:
            ...
            if len(season_cat_data) > 0:
                stats = {
                    ...
                    "avg_amount": float(
                        season_cat_data["Purchase Amount (USD)"].mean()
                    ),
                    "total_amount": float(
                        season_cat_data["Purchase Amount (USD)"].sum()
                    ),
                    "percentage": round(
                        len(season_cat_data) / len(category_data) * 100, 1
                    ),
                }
        ...
        if category_season_stats:
            ...
            best_by_revenue = max(
                category_season_stats, key=lambda x: x["total_amount"]
            )
            ...
            seasonal_performance.append(
                {
                    ...
                    "purchase_density": round(
                        (
                            season_data["total_purchases"]
                            / len(df[df["Season"] == season].index.unique())
                            if len(df[df["Season"] == season]) > 0
                            else 0
                        ),
                        2,
                    ),
    ...
    if seasonal_performance:
        ...
        cross_season_comparison = {
            ...
            "peak_season": {
                ...
                "reason": analyze_peak_season_reason(
                    seasonal_analysis[peak_season["season"]]
                ),
            },
            "low_season": {
            ...
            "revenue_variation": (
                round(
                    (peak_season["total_revenue"] - low_season["total_revenue"])
                    / low_season["total_revenue"]
                    * 100,
                    1,
                )
                if low_season["total_revenue"] > 0
                else 0
            ),
        }

    # 7. 季节性营销建议
    marketing_recommendations = generate_seasonal_recommendations(
        seasonal_analysis, seasonal_trends, highly_seasonal_categories
    )
    ...
    results = {
        ...
        "summary": {
            "peak_season": cross_season_comparison.get("peak_season", {}).get(
                "season", "Unknown"
            ),
            "most_consistent_category": find_most_consistent_category(seasonal_trends),
            "most_seasonal_category": (
                highly_seasonal_categories[0]["category"]
                if highly_seasonal_categories
                else "None"
            ),
            "highest_avg_transaction_season": (
                max(seasonal_performance, key=lambda x: x["avg_transaction_value"])[
                    "season"
                ]
                if seasonal_performance
                else "Unknown"
...
def analyze_peak_season_reason(season_data):
    ...
    for cat in top_categories:
        ...
        reasons.append(
            f"{category_name} ({cat['count']}次购买，占总数的{category_details.get('percentage', 0)}%)"
        )
...
def analyze_monthly_trends():
    ...
    if month_col:
        monthly_stats = (
            ...
            .agg({"Purchase Amount (USD)": ["count", "mean", "sum"]})
            .round(2)
        )
    ...
    for season, data in seasonal_analysis.items():
        if data["avg_transaction_value"] > 0:
            ...
            for category, details in data["category_details"].items():
                if (
                    details["avg_amount"] > data["avg_transaction_value"] * 1.2
                ):  # 高于平均20%
...
def analyze_review_rating_impact(input: str) -> dict:
    ...
    # 数据清洗
    df_clean = df.copy()
    df_clean["Review Rating"] = pd.to_numeric(
        df_clean["Review Rating"], errors="coerce"
    )
    df_clean = df_clean.dropna(subset=["Review Rating"])
    df_clean = df_clean[
        (df_clean["Review Rating"] >= 1) & (df_clean["Review Rating"] <= 5)
    ]
    ...
    df_clean["Rating Group"] = df_clean["Review Rating"].apply(
        create_simple_rating_groups
    )
    ...
    for group in ["High (4.0-5.0)", "Medium (3.0-3.99)", "Low (1.0-2.99)"]:
        if group in df_clean["Rating Group"].unique():
            ...
            rating_group_analysis[group] = {
                ...
                "avg_purchase_amount": round(
                    float(group_data["Purchase Amount (USD)"].mean()), 2
                ),
                "avg_previous_purchases": round(
                    float(group_data["Previous Purchases"].mean()), 1
                ),
                "total_revenue": round(
                    float(group_data["Purchase Amount (USD)"].sum()), 2
                ),
            }
    ...
    if len(df_clean) >= 10:
        try:
            ...
            corr_amount, p_value_amount = pearsonr(
                df_clean["Review Rating"], df_clean["Purchase Amount (USD)"]
            )

            # 评分与之前购买次数的相关性
            corr_prev, p_value_prev = pearsonr(
                df_clean["Review Rating"], df_clean["Previous Purchases"]
            )
            ...
            correlation_results = {
                "rating_vs_purchase_amount": {
                    ...
                    "strength": (
                        "强"
                        if abs(corr_amount) >= 0.5
                        else "中" if abs(corr_amount) >= 0.3 else "弱"
                    ),
                },
    ...
    if len(rating_group_analysis) >= 2:
        ...
        if high_group and low_group:
            amount_diff = (
                high_group["avg_purchase_amount"] - low_group["avg_purchase_amount"]
            )
            prev_diff = (
                high_group["avg_previous_purchases"]
                - low_group["avg_previous_purchases"]
            )

            key_comparisons = {
                "high_vs_low_rating": {
                    "purchase_amount_difference": round(amount_diff, 2),
                    "purchase_amount_percentage_diff": (
                        round(amount_diff / low_group["avg_purchase_amount"] * 100, 1)
                        if low_group["avg_purchase_amount"] > 0
                        else 0
                    ),
                    "previous_purchases_difference": round(prev_diff, 1),
                    "revenue_contribution_ratio": (
                        round(
                            high_group["total_revenue"] / low_group["total_revenue"], 1
                        )
                        if low_group["total_revenue"] > 0
                        else "N/A"
                    ),
                }
            }
    ...
    # 评分分布洞察
    high_rating_percentage = rating_group_analysis.get("High (4.0-5.0)", {}).get(
        "percentage", 0
    )
    ...
    if key_comparisons and "high_vs_low_rating" in key_comparisons:
        ...
        insights.append(
            f"高评分客户比低评分客户平均多消费${diff_info['purchase_amount_difference']:.2f} ({diff_info['purchase_amount_percentage_diff']:.1f}%)"
        )
    ...
    if correlation_results and "rating_vs_purchase_amount" in correlation_results:
        ...
        if corr_info["significant"]:
            ...
            insights.append(
                f"评分与消费金额存在{direction}相关关系({corr_info['strength']}相关，r={corr_info['correlation']:.2f})"
            )
    ...
    results = {
        "overall_summary": {
            ...
            "avg_purchase_amount": round(
                float(df_clean["Purchase Amount (USD)"].mean()), 2
            ),
            "avg_previous_purchases": round(
                float(df_clean["Previous Purchases"].mean()), 1
            ),
        },
        "rating_distribution": {
    ...
    try:
        ...
        counts = [
            rating_group_analysis.get(g, {}).get("customer_count", 0) for g in groups
        ]
    ...
    try:
        # 2) 各评分组平均消费金额柱状图
        avg_amounts = [
            rating_group_analysis.get(g, {}).get("avg_purchase_amount", 0)
            for g in groups
        ]
        plt.figure(figsize=(7, 4))
    ...
    try:
        # 4) 各评分组总收入柱状图
        totals = [
            rating_group_analysis.get(g, {}).get("total_revenue", 0) for g in groups
        ]
        plt.figure(figsize=(7, 4))
...
def analyze_payment_method_impact(input: str) -> dict:
    ...
    # 4. 关键对比：最高和最低平均金额
    avg_amounts = {
        method: stats["avg_amount"] for method, stats in payment_stats.items()
    }
    if avg_amounts:
        ...
        key_comparisons = {
            ...
            "difference": {
                "amount_diff": round(
                    avg_amounts[max_avg_method] - avg_amounts[min_avg_method], 2
                ),
                "percentage_diff": (
                    round(
                        (avg_amounts[max_avg_method] - avg_amounts[min_avg_method])
                        / avg_amounts[min_avg_method]
                        * 100,
                        1,
                    )
                    if avg_amounts[min_avg_method] > 0
                    else 0
                ),
            },
        }
    ...
    if len(payment_stats) >= 2:
        try:
            ...
            for method in payment_stats.keys():
                group_data = df_clean[df_clean["Payment Method"] == method][
                    "Purchase Amount (USD)"
                ].values
    ...
    for method, stats in payment_stats.items():
        contribution_analysis[method] = {
            ...
            "revenue_share": round(
                stats["total_amount"] / df_clean["Purchase Amount (USD)"].sum() * 100, 1
            ),
    ...
    insights.append(
        f"最常用的支付方式: {most_popular} ({payment_counts[most_popular]}笔交易)"
    )
    ...
    if key_comparisons:
        ...
        insights.append(
            f"{key_comparisons['highest_avg_payment']['method']}的平均交易额比{key_comparisons['lowest_avg_payment']['method']}高{diff['percentage_diff']:.1f}%"
        )
    ...
    for method, contrib in contribution_analysis.items():
        if (
            contrib["revenue_share"] > contrib["transaction_share"] + 10
        ):  # 收入占比明显高于交易占比
            insights.append(
                f"{method}是高价值支付方式：贡献{contrib['revenue_share']}%的收入，但只占{contrib['transaction_share']}%的交易"
            )
    ...
    results = {
        ...
        "payment_method_distribution": {
            ...
            "percentage_breakdown": {
                method: stats["percentage"] for method, stats in payment_stats.items()
            },
        },
    ...
    try:
        ...
        series_counts.plot(
            kind="pie", autopct="%1.1f%%", startangle=90, pctdistance=0.75
        )
        plt.ylabel("")
    ...
    try:
        ...
        plt.figure(figsize=(8, 5))
        methods_avg = (
            list(avg_amounts.keys())
            if "avg_amounts" in locals()
            else list(payment_stats.keys())
        )
        avg_vals = [
            payment_stats[m]["avg_amount"] if m in payment_stats else 0
            for m in methods_avg
        ]
    ...
    try:
        ...
        for method in payment_stats.keys():
            vals = (
                df_clean[df_clean["Payment Method"] == method]["Purchase Amount (USD)"]
                .dropna()
                .values
            )
            if len(vals) >= 3:
