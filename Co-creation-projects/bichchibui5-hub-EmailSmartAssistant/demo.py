...
class EmailDemo:
    ...
    def classify_email(self, email):
        ...
        spam_score = sum(
            1
            for keyword in self.classification_rules["spam_keywords"]
        ...
        work_score = sum(
            1
            for keyword in self.classification_rules["work_keywords"]
            if keyword in text_content
        )
        ...
        customer_score = sum(
            1
            for keyword in self.classification_rules["customer_keywords"]
            if keyword in text_content
        )
        ...
        personal_score = sum(
            1
            for keyword in self.classification_rules["personal_keywords"]
            if keyword in text_content
        )
        ...
        email_type = (
            max(scores, key=scores.get) if max(scores.values()) > 0 else "other"
        )

        # 确定优先级
        priority = (
            "high"
            if any(word in text_content for word in ["紧急", "urgent", "asap", "重要"])
            else "medium"
        )
        if email_type == "spam":
            priority = "low"
        ...
        return {"type": email_type, "priority": priority, "sender_type": sender_type}
    ...
    def extract_info(self, email):
        ...
        # 提取时间
        time_patterns = [r"\d{1,2}:\d{2}", r"\d{1,2}点", r"\d{1,2} PM", r"\d{1,2} AM"]
        ...
        emails = re.findall(
            r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b", body
        )
    ...
    def generate_reply(self, email, classification):
        ...
        template_type = (
            classification["type"]
            if classification["type"] in ["work", "customer"]
            else "general"
        )
    ...
    def display_results(self, results, stats):
        ...
        for i, result in enumerate(results[:3], 1):  # 显示前3个
            ...
            print(
                f"  分类: {classification['type']} | 优先级: {classification['priority']}"
            )
