...
class LLMConfig:
    ...
    model_name: str = (
        "gpt-3.5-turbo"  # OpenAI: gpt-4, gpt-3.5-turbo, gpt-4-turbo-preview
    )
...
class InnoCoreConfig:
    ...
    def __post_init__(self):
        ...
        self.database.password = self.database.password or os.getenv(
            "DATABASE_PASSWORD"
        )
        self.redis.password = self.redis.password or os.getenv("REDIS_PASSWORD")

        self.external_apis.crossref_api_key = (
            self.external_apis.crossref_api_key or os.getenv("CROSSREF_API_KEY")
        )
        self.external_apis.google_scholar_api_key = (
            self.external_apis.google_scholar_api_key
            or os.getenv("GOOGLE_SCHOLAR_API_KEY")
        )
        self.external_apis.serpapi_key = self.external_apis.serpapi_key or os.getenv(
            "SERPAPI_KEY"
        )
