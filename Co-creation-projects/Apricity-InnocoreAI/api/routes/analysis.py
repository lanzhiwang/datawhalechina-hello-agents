...
async def analyze_paper(request: PaperAnalysisRequest):
    ...
    try:
        if not llm:
            raise HTTPException(
                status_code=503, detail="AI 服务未配置，请设置 OPENAI_API_KEY"
            )
        ...
        if paper_url.startswith("/uploads/") or paper_url.endswith(".pdf"):
            ...
            if paper_url.startswith("/uploads/"):
                ...
                file_path = os.path.join(
                    "downloads", paper_url.replace("/uploads/", "")
                )
            else:
                file_path = paper_url
            ...
            if not os.path.exists(file_path):
                ...
                raise HTTPException(
                    status_code=404, detail=f"PDF 文件不存在: {paper_url}"
                )
            ...
            if not pdf_result.get("success"):
                raise HTTPException(
                    status_code=500, detail=pdf_result.get("error", "PDF 解析失败")
                )
...
请用中文回答，保持专业和深度。""",
        ...
        if not paper:
            raise HTTPException(
                status_code=404, detail=f"未找到 ArXiv 论文: {paper_id}"
            )
    ...
    try:
        ...
        for paper_id in paper_ids:
            try:
                ...
                results.append(
                    {"paper_id": paper_id, "success": False, "error": str(e)}
                )
