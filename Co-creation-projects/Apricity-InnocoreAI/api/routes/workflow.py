...
async def complete_workflow(request: WorkflowRequest):
    ...
    try:
        ...
        results = {"workflow_id": workflow_id, "status": "running", "steps": []}
        ...
        try:
            ...
            search_result = await search_papers(
                PaperSearchRequest(
                    keywords=request.keywords, source="arxiv", limit=request.limit
                )
            )
            ...
            results["steps"].append(
                {
                    ...
                    "name": "Hunter - 论文搜索",
                    "status": "completed",
                    "result": {"total_found": len(papers), "papers": papers},
                }
            )
                ...
                try:
                    analysis_result = await analyze_paper(
                        PaperAnalysisRequest(
                            paper_url=paper["url"], analysis_type=request.analysis_type
                        )
                    )
                    analyses.append(
                        {
        ...
        citations = []
        try:
            from api.routes.citations import (
                validate_citation,
            ...
            for paper in papers[:3]:
                try:
                    ...
                    citation_result = await validate_citation(
                        CitationValidationRequest(
                            citation=citation_text, format=request.citation_format
                        )
                    )
                    ...
                    citations.append(
                        {
                            "paper_id": paper["id"],
                            "title": paper["title"],
                            "formatted_citation": citation_result.get(
                                "formatted_citation", ""
                            ),
                        }
                    )
        ...
        if request.writing_task:
            ...
            try:
                ...
                # 使用 Coach 改进报告
                writing_result = await writing_coach(
                    WritingCoachRequest(
                        text=report_text, style="academic", task=request.writing_task
                    )
                )
                ...
                results["steps"].append(
                    {
                        ...
                        "name": "Coach - 报告生成",
                        "status": "completed",
                        "result": {"report": writing_result.get("result", "")},
                    }
                )
...
async def search_and_analyze(request: WorkflowRequest):
    ...
    try:
        ...
        papers = search_result.get("papers", [])
        results["steps"].append(
            {"step": 1, "name": "搜索论文", "status": "completed", "papers": papers}
        )
        ...
        analysis_result = await analyze_paper(
            PaperAnalysisRequest(
                paper_url=first_paper["url"], analysis_type=request.analysis_type
            )
        )
