                ...
                async with httpx.AsyncClient() as client:
                    try:
                        response = await client.get(
                            f"https://api.crossref.org/works/{doi}", timeout=10.0
                        )
...
{{"title": "论文标题", "authors": ["作者1", "作者2"], "year": 2024, "journal": "期刊名"}}"""
        ...
        if metadata and verified:
            ...
            authors = (
                ...
                else [metadata.get("authors", "Unknown Author")]
            )
        ...
        result = {
            ...
            "warnings": (
                ...
                if verified
                    ...
                    "无法自动验证引用，已返回原始格式。建议提供包含 DOI 的引用信息以获得更准确的结果。"
                ]
            ),
        }

        logger.info(
            f"返回结果 - verified: {verified}, metadata: {metadata is not None}"
        )
        if metadata:
