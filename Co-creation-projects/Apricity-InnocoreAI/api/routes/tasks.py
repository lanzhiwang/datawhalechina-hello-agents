...
async def submit_task(request: TaskSubmitRequest):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"不支持的任务类型: {request.task_type}"
            )
...
async def handle_writing_assistance(websocket: WebSocket, data: Dict[str, Any]):
    ...
    try:
        # 提交写作辅助任务
        task_id = await agent_controller.submit_task(
            task_type=TaskType.WRITING_ASSISTANCE, input_data=data
        )

        # 发送任务ID
        await manager.send_personal_message(
            json.dumps({"type": "task_started", "task_id": task_id}), websocket
        )
        ...
        # 发送结果
        await manager.send_personal_message(
            json.dumps({"type": "task_completed", "result": result}), websocket
        )
