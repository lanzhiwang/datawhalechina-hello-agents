...
async def create_user(request: UserCreateRequest):
    ...
    try:
        user_id = await db_manager.create_user(
            email=request.email, profile=request.profile
        )
...
async def update_user(user_id: str, request: UserUpdateRequest):
    ...
    try:
        success = await db_manager.update_user_profile(
            user_id=user_id, profile=request.profile
        )
...
async def get_user_profile(user_id: str):
    ...
    try:
        ...
        return {"success": True, "profile": user.get("profile", {})}
...
async def update_user_profile(user_id: str, profile: Dict[str, Any]):
    """更新用户配置"""
    try:
        success = await db_manager.update_user_profile(user_id=user_id, profile=profile)
