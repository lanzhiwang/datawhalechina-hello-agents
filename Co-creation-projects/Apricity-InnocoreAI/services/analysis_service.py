...
class AnalysisService:
    ...
    def get_analysis_by_id(self, analysis_id: int) -> Optional[Analysis]:
        """根据ID获取分析"""
        analysis_db = (
            self.db.query(AnalysisDB).filter(AnalysisDB.id == analysis_id).first()
        )
        if not analysis_db:
    ...
    def get_analyses_by_user(
        ...
        analyses_db = (
            ...
            .filter(AnalysisDB.user_id == user_id)
    ...
    def update_analysis(
        self, analysis_id: int, analysis_update: AnalysisUpdate
    ) -> Analysis:
        """更新分析"""
        analysis_db = (
            self.db.query(AnalysisDB).filter(AnalysisDB.id == analysis_id).first()
    ...
    def get_analysis_statistics(self, user_id: int) -> Dict[str, Any]:
        """获取分析统计信息"""
        total_analyses = (
            self.db.query(AnalysisDB).filter(AnalysisDB.user_id == user_id).count()
        )

        # 按类型统计
        type_stats = (
            self.db.query(AnalysisDB.analysis_type, self.db.func.count(AnalysisDB.id))
            .filter(AnalysisDB.user_id == user_id)
            .group_by(AnalysisDB.analysis_type)
            .all()
        )
    ...
    def _export_to_markdown(self, analysis: Analysis) -> str:
        ...
        markdown += (
            f"**创建时间**: {analysis.created_at.strftime('%Y-%m-%d %H:%M:%S')}\n\n"
        )
