...
class TaskService:
    ...
    def cancel_task(self, task_id: int) -> Task:
        ...
        return self.update_task(
            task_id, TaskUpdate(status="failed", error_message="Task cancelled by user")
        )
    ...
    def retry_task(self, task_id: int) -> Task:
        ...
        task = self.update_task(
            task_id, TaskUpdate(status="pending", progress=0, error_message=None)
        )
    ...
    def get_task_statistics(self, user_id: int) -> Dict[str, Any]:
        ...
        # 按状态统计
        status_stats = (
            self.db.query(TaskDB.status, self.db.func.count(TaskDB.id))
            .filter(TaskDB.user_id == user_id)
        ...
        # 按类型统计
        type_stats = (
            self.db.query(TaskDB.task_type, self.db.func.count(TaskDB.id))
            .filter(TaskDB.user_id == user_id)
            .group_by(TaskDB.task_type)
            .all()
        )
        ...
        completed_tasks = (
            self.db.query(TaskDB)
            .filter(and_(TaskDB.user_id == user_id, TaskDB.status == "completed"))
            .count()
        )
        ...
        for paper_data in enriched_results:
            try:
                paper = paper_service.create_paper(
                    PaperCreate(**paper_data), task_db.user_id
                )
    ...
    async def _execute_writing(self, task_db: TaskDB) -> Dict[str, Any]:
        ...
        # 生成内容
        writing_result = await coach_agent.generate_writing(
            paper_ids, writing_type, outline
        )
