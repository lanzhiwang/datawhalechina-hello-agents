...
class PaperService:
    ...
    def get_papers_by_user(
        ...
        papers_db = (
            ...
            .filter(PaperDB.user_id == user_id)
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def create_paper(self, paper_create: PaperCreate, user_id: int) -> Paper:
        ...
        if paper_create.doi:
            existing = (
                self.db.query(PaperDB).filter(PaperDB.doi == paper_create.doi).first()
            )
            if existing:
                raise PaperAlreadyExistsError(
                    f"Paper with DOI {paper_create.doi} already exists"
                )
        ...
        if paper_create.arxiv_id:
            existing = (
                self.db.query(PaperDB)
                .filter(PaperDB.arxiv_id == paper_create.arxiv_id)
                .first()
            )
            if existing:
                raise PaperAlreadyExistsError(
                    f"Paper with arXiv ID {paper_create.arxiv_id} already exists"
                )
    ...
    def semantic_search(self, query: str, user_id: int, limit: int = 10) -> List[Paper]:
        ...
        papers_db = (
            self.db.query(PaperDB)
            .filter(and_(PaperDB.id.in_(paper_ids), PaperDB.user_id == user_id))
            .all()
        )
    ...
    def _process_paper_async(self, paper_id: int):
        ...
        try:
            ...
            self.vector_store.add_document(
                doc_id=paper_id,
                embedding=embedding,
                metadata={"title": paper_db.title, "user_id": paper_db.user_id},
            )
    ...
    def get_paper_statistics(self, user_id: int) -> Dict[str, Any]:
        ...
        processed_papers = (
            self.db.query(PaperDB)
            .filter(and_(PaperDB.user_id == user_id, PaperDB.is_processed == True))
            .count()
        )

        # 按年份统计
        year_stats = (
            self.db.query(PaperDB.publication_year, self.db.func.count(PaperDB.id))
            .filter(PaperDB.user_id == user_id)
            .group_by(PaperDB.publication_year)
            .all()
        )

        # 按期刊统计
        journal_stats = (
            self.db.query(PaperDB.journal, self.db.func.count(PaperDB.id))
            .filter(PaperDB.user_id == user_id)
            .group_by(PaperDB.journal)
            .all()
        )
        ...
        return {
            ...
            "processing_rate": (
                processed_papers / total_papers if total_papers > 0 else 0
            ),
