...
class WritingService:
    ...
    def get_writings_by_user(
        self, user_id: int, skip: int = 0, limit: int = 20
    ) -> List[Writing]:
        """获取用户的写作列表"""
        writings_db = (
            self.db.query(WritingDB)
            .filter(WritingDB.user_id == user_id)
            .order_by(WritingDB.created_at.desc())
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def create_writing(
        self, writing_create: WritingCreate, user_id: int, task_id: Optional[int] = None
    ) -> Writing:
        """创建写作"""
        # 计算字数
    ...
    def get_writing_statistics(self, user_id: int) -> Dict[str, Any]:
        """获取写作统计信息"""
        total_writings = (
            self.db.query(WritingDB).filter(WritingDB.user_id == user_id).count()
        )

        # 按类型统计
        type_stats = (
            self.db.query(WritingDB.writing_type, self.db.func.count(WritingDB.id))
            .filter(WritingDB.user_id == user_id)
            .group_by(WritingDB.writing_type)
            .all()
        )

        # 按状态统计
        status_stats = (
            self.db.query(WritingDB.status, self.db.func.count(WritingDB.id))
            .filter(WritingDB.user_id == user_id)
            .group_by(WritingDB.status)
            .all()
        )

        # 总字数
        total_words = (
            self.db.query(self.db.func.sum(WritingDB.word_count))
            .filter(WritingDB.user_id == user_id)
            .scalar()
            or 0
        )
        ...
        # 平均质量分数
        avg_quality = (
            self.db.query(self.db.func.avg(WritingDB.quality_score))
            .filter(WritingDB.user_id == user_id)
            .scalar()
            or 0
        )
    ...
    def format_citations(self, writing_id: int, style: str = "APA") -> Dict[str, Any]:
        ...
        return {
            ...
            "bibliography": self.citation_formatter.generate_bibliography(
                papers, style
            ),
        }

    def export_writing(
        self, writing_id: int, format: str = "markdown", include_citations: bool = True
    ) -> Dict[str, Any]:
        """导出写作"""
        writing = self.get_writing_by_id(writing_id)
    ...
    def _export_to_markdown(
        self, writing: Writing, include_citations: bool = True
    ) -> str:
        """导出为Markdown格式"""
        ...
        markdown += (
            f"**创建时间**: {writing.created_at.strftime('%Y-%m-%d %H:%M:%S')}\n\n"
        )
    ...
    def generate_outline(
        self, topic: str, paper_ids: List[int], writing_type: str = "review"
    ) -> List[Dict[str, Any]]:
        """生成写作大纲"""
        # 获取参考论文
        papers = []
