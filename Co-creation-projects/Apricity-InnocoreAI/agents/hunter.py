...
class HunterAgent(BaseAgent):
            ...
            for paper in filtered_papers[:max_papers]:
                try:
                    ...
                    self._add_to_history(
                        f"下载论文失败 {paper.get('title', 'Unknown')}: {str(e)}"
                    )
    ...
    async def _search_papers_from_arxiv(
        self, keywords: List[str], max_papers: int, days_back: int
    ) -> List[Dict]:
        """从ArXiv搜索论文"""
        papers = []
                ...
                async with session.get(self.arxiv_base_url, params=params) as response:
                    if response.status != 200:
                        raise ExternalAPIException(
                            f"ArXiv API请求失败: {response.status}"
                        )
    ...
    async def _search_papers_from_ieee(
        self, keywords: List[str], max_papers: int, days_back: int
    ) -> List[Dict]:
        """从IEEE搜索论文"""
        papers = []
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(self.ieee_base_url, params=params) as response:
                    if response.status != 200:
                        raise ExternalAPIException(
                            f"IEEE API请求失败: {response.status}"
                        )
                    ...
                    for article in data.get("articles", []):
                        paper = {
                            ...
                            "source": "ieee",
                            "doi": article.get("doi", ""),
                            "categories": article.get("index_terms", {})
                            .get("ieee_terms", {})
                            .get("terms", []),
                        }
    ...
    async def _filter_papers(
        self, papers: List[Dict], keywords: List[str]
    ) -> List[Dict]:
        """根据关键词筛选论文"""
        filtered_papers = []
    ...
    async def _save_paper_to_db(self, paper: Dict):
        ...
        try:
            # 检查是否已存在
            existing_paper = await db_manager.get_paper_by_hash(
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(pdf_url) as response:
                    if response.status == 200:
                        content = await response.read()
                        filename = (
                            f"download_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
                        )
                        file_path = os.path.join(self.download_dir, filename)
