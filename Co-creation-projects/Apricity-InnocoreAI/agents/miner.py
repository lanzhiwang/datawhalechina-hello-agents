...
class MinerAgent(BaseAgent):
    ...
    async def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_type = input_data.get(
                "analysis_type", "full"
            )  # full, quick, innovation_only
            ...
            related_papers = await self._find_related_papers(
                paper["title"], paper["abstract"], user_id
            )
    ...
    async def _find_related_papers(
        self, title: str, abstract: str, user_id: str = None
    ) -> List[Dict]:
        """查找相关论文"""
        try:
            # 构建查询
    ...
    async def _perform_comparison_analysis(
        self, current_paper: Dict, related_papers: List[Dict]
    ) -> Dict[str, Any]:
        """执行对比分析"""
    ...
    async def _create_analysis_report(
        ...
        try:
            ...
            try:
                ...
                # 如果JSON解析失败，生成默认报告
                report = self._generate_default_report(
                    paper, parsed_content, comparison_result
                )
    ...
    def _generate_default_report(
        self, paper: Dict, parsed_content: Dict, comparison_result: Dict
    ) -> Dict[str, Any]:
        """生成默认报告"""
        return {
    ...
    async def _save_analysis_report(
        self, paper_id: str, report: Dict, user_id: str = None
    ) -> str:
        """保存分析报告到数据库"""
        try:
            report_id = await db_manager.create_analysis_report(
                ...
                innovation_point=json.dumps(
                    report.get("innovation_points", []), ensure_ascii=False
                ),
                limitation=json.dumps(
                    report.get("limitations", []), ensure_ascii=False
                ),
                future_idea=json.dumps(
                    report.get("future_ideas", []), ensure_ascii=False
                ),
    ...
    async def _update_vector_store(
        self, paper_id: str, paper: Dict, parsed_content: Dict, user_id: str = None
    ):
        """更新向量库"""
        try:
            title = paper.get("title", "")
