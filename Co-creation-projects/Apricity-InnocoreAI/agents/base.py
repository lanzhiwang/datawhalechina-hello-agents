...
class BaseAgent(ABC):
    ...
    def __init__(self, name: str, llm=None, max_steps: int = None, timeout: int = None):
        self.name = name
    ...
    def add_tool(self, tool_name: str, tool_func: Callable, description: str = ""):
        """添加工具"""
        self.tools[tool_name] = {"function": tool_func, "description": description}
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(tool_func):
                result = await asyncio.wait_for(
                    tool_func(tool_input), timeout=self.timeout
                )
            else:
                result = await asyncio.wait_for(
    ...
    async def think(self, prompt: str, context: Dict = None) -> str:
        ...
        try:
            ...
            response = await asyncio.wait_for(
                self.llm.ainvoke(full_prompt), timeout=self.timeout
            )
