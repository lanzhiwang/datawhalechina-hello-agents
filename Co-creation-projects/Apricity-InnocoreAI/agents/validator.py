...
class ValidatorAgent(BaseAgent):
    ...
    async def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            final_citations = await self._merge_citation_data(
                citations, verification_result, paper_info
            )
    ...
    async def _generate_citations(
        self, paper_info: Dict, formats: List[str]
    ) -> Dict[str, Any]:
        """生成多种格式的引用"""
        citations = {}
        ...
        for format_type in formats:
            try:
                if format_type.lower() == "bibtex":
                    citations["bibtex"] = await self._generate_bibtex_citation(
                        paper_info
                    )
    ...
    async def _verify_paper_metadata(self, paper_info: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # 确定最终状态
            if (
                verification_result["crossref_verified"]
                or verification_result["scholar_verified"]
            ):
                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        self._add_to_history(
                            f"CrossRef查询失败，状态码: {response.status}"
                        )
                        return None
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    self.google_scholar_url, params=params
                ) as response:
                    if response.status == 200:
                        data = await response.json()
                        ...
                        self._add_to_history(
                            f"Google Scholar查询失败，状态码: {response.status}"
                        )
                        return None
    ...
    def _parse_scholar_data(self, data: Dict) -> Dict:
        ...
        return {
            ...
            "year": year,
            "journal": (
                publication_info.get("summary", "").split(",")[0]
    ...
    def _compare_metadata(self, original: Dict, reference: Dict) -> List[Dict]:
        ...
        if orig_year and ref_year and orig_year != ref_year:
            discrepancies.append(
                {"field": "year", "original": orig_year, "reference": ref_year}
            )
    ...
    async def _merge_citation_data(
        self, citations: Dict, verification: Dict, paper_info: Dict
    ) -> Dict[str, Any]:
        """合并引用数据"""
        final_citations = {}

        for format_type, citation_text in citations.items():
            if isinstance(citation_text, str) and not citation_text.startswith(
                "生成失败"
            ):
                # 添加校验状态标记
            ...
            if doi:
                ...
                if bibtex and not bibtex.startswith("生成失败"):
                    ...
                    await db_manager.cache_reference(
                        doi=doi, bibtex=bibtex, is_verified=is_verified
                    )
