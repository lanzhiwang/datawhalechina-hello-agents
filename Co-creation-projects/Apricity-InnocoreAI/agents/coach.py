...
class CoachAgent(BaseAgent):
    ...
    async def _handle_explain_task(
        self, user_id: str, content: str, context: Dict
    ) -> Dict[str, Any]:
        """处理解释任务"""
        try:
            # 获取用户的历史论文作为上下文
    ...
    async def _handle_suggest_task(
        self, user_id: str, content: str, context: Dict
    ) -> Dict[str, Any]:
        """处理建议任务"""
        try:
    ...
    async def _get_style_references(
    ...
    async def _get_user_top_papers(
    ...
    async def _explain_concept(self, concept: str, context: Dict = None) -> Dict:
        ...
        return await self._handle_explain_task(
            context.get("user_id", ""), concept, context or {}
        )

    async def _polish_text(self, text: str, context: Dict = None) -> Dict:
        """润色文本工具"""
        return await self._handle_polish_task(
            context.get("user_id", ""), text, context or {}
        )

    async def _mimic_style(
        self, text: str, target_style: str, context: Dict = None
    ) -> Dict:
        """模仿风格工具"""
        ctx = context or {}
        ctx["target_style"] = target_style
        return await self._handle_mimic_task(ctx.get("user_id", ""), text, ctx)
