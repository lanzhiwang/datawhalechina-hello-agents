...
def count_dimension_frequency(records: List[Dict]) -> Dict[str, Dict]:
    ...
    dimension_stats = defaultdict(
        lambda: {"frequency": 0, "dates": [], "first_seen": None, "last_seen": None}
    )
        ...
        for dim in dimensions:
            if dim:  # 跳过空字符串
                ...
                if record_date:
                    ...
                    if (
                        dimension_stats[dim]["first_seen"] is None
                        or record_date < dimension_stats[dim]["first_seen"]
                    ):
                        dimension_stats[dim]["first_seen"] = record_date
                    if (
                        dimension_stats[dim]["last_seen"] is None
                        or record_date > dimension_stats[dim]["last_seen"]
                    ):
    ...
    for dim, stats in dimension_stats.items():
        result[dim] = {
            ...
            "frequency_rate": (
                stats["frequency"] / total_records if total_records > 0 else 0.0
            ),
            "first_seen": (
            ...
            "last_seen": (
                stats["last_seen"].strftime("%Y-%m-%d") if stats["last_seen"] else None
            ),
...
def find_missing_dimensions(
    records: List[Dict], candidate_dimensions: List[str], days_threshold: int = 30
) -> List[str]:
    """查找缺失的维度（超过N天未出现）"""
    now = datetime.now()
    missing = []
...
def suggest_add_dimensions(
    records: List[Dict], candidate_dimensions: List[str], threshold_days: int = 30
) -> List[Dict]:
    """建议添加缺失但重要的维度"""
...
def suggest_remove_dimensions(
    records: List[Dict], active_dimensions: List[str], threshold_days: int = 60
) -> List[Dict]:
    """建议删除长期未出现的维度"""
    stats = count_dimension_frequency(records)
    suggestions = []
...
def suggest_priority_adjustment(
    records: List[Dict], dimension_config: Dict
) -> List[Dict]:
    """建议调整频繁出现维度的优先级"""
    stats = count_dimension_frequency(records)
...
def generate_all_suggestions(
    ...
    # 获取当前配置
    active_dimensions = [
        d["name"] for d in dimension_config.get("active_dimensions", [])
    ]
    candidate_dimensions = dimension_config.get("candidate_dimensions", [])

    # 生成各类建议
    add_suggestions = suggest_add_dimensions(
        all_records, candidate_dimensions, threshold_days=30
    )
    remove_suggestions = suggest_remove_dimensions(
        all_records, active_dimensions, threshold_days=60
    )
...
def count_dimension_frequency_from_extractions(
    ...
    for result in extraction_results:
        ...
        extraction_date_str = result.get(
            "extraction_date", result.get("report_date", "")
        )
        ...
        for dim in dimensions:
            if dim:
                ...
                if extraction_date:
                    ...
                    if (
                        dimension_stats[dim]["first_seen"] is None
                        or extraction_date < dimension_stats[dim]["first_seen"]
                    ):
                        dimension_stats[dim]["first_seen"] = extraction_date
                    if (
                        dimension_stats[dim]["last_seen"] is None
                        or extraction_date > dimension_stats[dim]["last_seen"]
                    ):
    ...
    for dim, stats in dimension_stats.items():
        result[dim] = {
            "frequency": stats["frequency"],
            "frequency_rate": (
                stats["frequency"] / total_extractions if total_extractions > 0 else 0.0
            ),
            "first_seen": (
                stats["first_seen"].strftime("%Y-%m-%d")
                if stats["first_seen"]
...
def analyze_theme_dimension_match(
    themes: List[str], extraction_results: List[Dict], days_window: int = 30
) -> Dict[str, Dict]:
    """分析themes与维度的匹配度"""
    now = datetime.now()
    ...
    for theme in themes:
        ...
        theme_match[theme] = {
            ...
            "last_match_date": (
                last_match_date.strftime("%Y-%m-%d") if last_match_date else None
            ),
            "days_without_match": (
                (now - last_match_date).days if last_match_date else days_window
            ),
        }
...
def suggest_add_themes(
    dim_stats: Dict[str, Dict],
    themes: List[str],
    threshold_frequency: float = 0.5,
            ...
            if last_seen_str:
                ...
                if last_seen:
                    ...
                    if days_diff <= days_window:
                        # 估算最近出现次数（简化：假设频率一致）
                        recent_count = int(
                            frequency_rate * (days_window / 7)
                        )  # 粗略估算

            # 如果频率达到阈值且最近有出现
            if (
                frequency_rate >= threshold_frequency
                and recent_count >= min_recent_count
            ):
                suggestions.append(
                    {
...
def suggest_remove_themes(
    theme_match: Dict[str, Dict], threshold_frequency: float = 0.1, min_days: int = 60
) -> List[Dict]:
...
def generate_theme_suggestions(
    extraction_results: List[Dict], themes: List[str]
) -> Dict[str, List[Dict]]:
    """生成themes修正建议"""
    # 统计维度频率
    dim_stats = count_dimension_frequency_from_extractions(extraction_results)
    ...
    # 分析themes匹配度（使用30天窗口）
    theme_match = analyze_theme_dimension_match(
        themes, extraction_results, days_window=30
    )

    # 生成建议
    add_suggestions = suggest_add_themes(
        dim_stats, themes, threshold_frequency=0.5, min_recent_count=3, days_window=30
    )
    remove_suggestions = suggest_remove_themes(
        theme_match, threshold_frequency=0.1, min_days=60
    )
...
def record_dimension_event(
    event_type: str, dimension: str, timestamp: str = None, metadata: Dict = None
) -> Dict:
    """记录维度事件（ADD/REMOVE/PRIORITY_CHANGE）"""
    if timestamp is None:
        timestamp = datetime.now().strftime("%Y-%m-%d")

    event = {"date": timestamp, "event": event_type, "dimension": dimension}
...
def present_suggestions(suggestions: Dict[str, List[Dict]]) -> None:
    ...
    if suggestions.get("priority_adjustment"):
        ...
        for i, sug in enumerate(suggestions["priority_adjustment"], 1):
            ...
            print(f"     原因: {sug['reason']}")
            print(
                f"     当前优先级: {sug['current_priority']:.2f} → 建议: {sug['suggested_priority']:.2f}"
            )
