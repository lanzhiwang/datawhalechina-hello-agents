...
def present_theme_suggestions(suggestions: Dict[str, List[Dict]]):
    ...
    all_count = sum(
        len(v) for k, v in suggestions.items() if k != "theme_match_analysis"
    )
    if all_count == 0:
        print("✅ 暂无themes修正建议")
        return
...
def get_batch_user_confirmation(
    add_suggestions: List[Dict], remove_suggestions: List[Dict]
) -> Dict[str, List[int]]:
    """批量获取用户确认
        ...
        while True:
            ...
            try:
                ...
                if len(valid_numbers) != len(numbers):
                    ...
                    print(
                        f"⚠️  序号 {invalid} 超出范围（1-{len(add_suggestions)}），已忽略"
                    )
    ...
    if remove_suggestions:
        ...
        while True:
            ...
            try:
                ...
                # 验证序号范围
                valid_numbers = [
                    n for n in numbers if 1 <= n <= len(remove_suggestions)
                ]
                if len(valid_numbers) != len(numbers):
                    invalid = [
                        n for n in numbers if n < 1 or n > len(remove_suggestions)
                    ]
                    print(
                        f"⚠️  序号 {invalid} 超出范围（1-{len(remove_suggestions)}），已忽略"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="维度分析工具 - 从报告中提取维度并修正themes"
    )
    parser.add_argument(
        "--extract", action="store_true", help="重新提取维度（从报告文件中）"
    )
    parser.add_argument(
        "--interactive", action="store_true", help="交互模式：展示建议并获取用户确认"
    )
    parser.add_argument(
        "--base-dir", type=str, default=None, help="基础目录路径（默认为脚本所在目录）"
    )
    ...
    if args.extract:
        ...
        extraction_results = ed.batch_extract_dimensions(
            base_dir, report_type=None, llm=llm, existing_themes=existing_themes
        )
    ...
    if dim_stats:
        print("   维度频率（Top 5）:")
        sorted_dims = sorted(
            dim_stats.items(), key=lambda x: x[1]["frequency"], reverse=True
        )[:5]
        for dim, stats in sorted_dims:
            print(
                f"   - {dim}: {stats['frequency']}次 ({stats['frequency_rate']*100:.1f}%)"
            )
    ...
    total_suggestions = len(suggestions.get("add", [])) + len(
    ...
    if args.interactive and total_suggestions > 0:
        ...
        add_suggestions = suggestions.get("add", [])
        remove_suggestions = suggestions.get("remove", [])
        selected_indices = get_batch_user_confirmation(
            add_suggestions, remove_suggestions
        )

        # 应用用户选择的建议
        updated_themes = apply_theme_suggestions(
            suggestions, themes, themes_file, selected_indices
        )
