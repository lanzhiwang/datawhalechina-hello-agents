...
def init_llm():
    ...
    # 从环境变量读取LLM配置
    llm_model = os.getenv("LLM_MODEL") or os.getenv("LLM_MODEL_ID") or "qwen-plus"
    llm_api_key = (
    ...
    llm_base_url = (
        os.getenv("LLM_BASE_URL") or "https://api-inference.modelscope.cn/v1/"
    )
...
def extract_dimensions_from_text(
    text: str, llm, existing_themes: List[str] = None
) -> Dict:
    """从报告文本中提取维度
...
请直接返回JSON，不要包含其他文字。"""
    ...
    try:
        messages = [
            {
                "role": "system",
                "content": "你是一个专业的文本分析助手，擅长从文本中提取高级别的主题和兴趣维度。你会理解语义上下文，保持概念的完整性，不会简单地进行分词。",
            },
...
def extract_dimensions_from_report(
    report_file: Path, llm, existing_themes: List[str] = None
) -> Optional[Dict]:
    """从Markdown文件中提取维度
    ...
    try:
        ...
        result = extract_dimensions_from_text(
            text, llm, existing_themes=existing_themes
        )
...
def batch_extract_dimensions(
    base_dir: Path, report_type: str = None, llm=None, existing_themes: List[str] = None
) -> List[Dict]:
    """批量提取维度
    ...
    if existing_themes is None:
        try:
            ...
            if themes_file.exists():
                ...
                if existing_themes:
                    print(
                        f"📌 已加载 {len(existing_themes)} 个现有themes作为参考: {existing_themes}"
                    )
        except Exception as e:
    ...
    for rtype in types_to_process:
        ...
        for report_file in report_files:
            ...
            result = extract_dimensions_from_report(
                report_file, llm, existing_themes=existing_themes
            )
            ...
            if result and result.get("dimensions"):
                ...
                print(
                    f"    ✅ 提取到 {len(result['dimensions'])} 个维度: {', '.join(result['dimensions'][:5])}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--report-file", type=str, help="指定单个报告文件路径")
    parser.add_argument(
    ...
    if args.report_file:
        ...
        # 批量处理
        results = batch_extract_dimensions(
            base_dir, args.report_type, llm, existing_themes=existing_themes
        )
        print(f"\n✅ 共处理 {len(results)} 个报告")
