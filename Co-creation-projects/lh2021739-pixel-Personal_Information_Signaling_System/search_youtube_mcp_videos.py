...
def init_research_llm():
    ...
    llm_model = (
        ...
        or "Qwen/Qwen2.5-7B-Instruct"  # 默认通义千问模型
    )
    llm_api_key = (
        os.getenv("LLM_API_KEY")  # 优先使用 LLM_API_KEY（阿里云通义千问）
        or os.getenv("MODELSCOPE_API_KEY")
        or os.getenv("MODELSCOPE_API_TOKEN")
    )
    llm_base_url = (
        os.getenv("LLM_BASE_URL")
        or "https://api-inference.modelscope.cn/v1/"  # ModelScope 默认地址
    )
...
{{
  ...
  "next_steps": [
    ...
    messages = [
        {
            "role": "system",
            "content": "你是一位专业的研究分析师，擅长从视频信息中提取关键洞察并给出可执行的行动建议。请始终以 JSON 格式返回结果。",
        },
    ...
    try:
        ...
        missing_fields = [
            field for field in required_fields if field not in research_data
        ]
        if missing_fields:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="YouTube 视频搜索 - 多主题智能搜索与日报生成"
    )
    parser.add_argument(
        "--mode",
        type=str,
    ...
    filtered_videos = [
        ...
        if is_within_time_window(v["published_at"], DAYS_WINDOW)
    ]
    ...
    for video in filtered_videos:
        ...
        video["scoring_details"] = {
            "whitelist_bonus": (
                10 if video["channel_title"] in whitelist_channels else 0
            ),
            "keyword_matches": count_theme_keywords(video["title"], themes)
            + count_theme_keywords(video["description"], themes),
            ...
            try:
                ...
                if research_report:
                    ...
                    for i, finding in enumerate(
                        research_report.get("key_findings", []), 1
                    ):
                        print(f"  {i}. {finding}")
                    print(
